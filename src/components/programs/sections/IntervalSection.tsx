...
export default function IntervalSection({ section, sport, onChange }: Props) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-4 p-3 bg-surface-light rounded-xl border border-cb-border">
        ...
        <CtrlField label="Rest (s)">
          <input
            type="number"
            min={0}
            value={section.restSeconds ?? 20}
            onChange={e => onChange({ ...section, restSeconds: Number(e.target.value) })}
            className={ctrlInput}
          />
        </CtrlField>
        {section.rounds && section.workSeconds && section.restSeconds && (
          <div className="text-xs text-cb-muted">
            = {Math.round((section.rounds * (section.workSeconds + section.restSeconds)) / 60)} min total
          </div>
        )}
      </div>
