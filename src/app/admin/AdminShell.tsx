...
function AdminPasswordPrompt() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true)
    try {
      // PIN is verified server-side; success sets an HttpOnly cookie that
      // admin/layout.tsx reads on the reload.
      ...
      <div className="w-full max-w-md p-8 bg-slate-800 rounded-xl border border-slate-700 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={submitting}
            className="w-full px-4 py-3 bg-teal text-white font-semibold rounded-lg hover:bg-teal/90 transition-colors disabled:opacity-60"
          >
