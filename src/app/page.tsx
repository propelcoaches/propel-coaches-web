...
function AnimatedSection({ children, className = '', style }: { children: ReactNode; className?: string; style?: CSSProperties }) {
  ...
  return (
    <div
      ref={ref}
      style={style}
      className={`transition-all duration-700 ${isInView ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'} ${className}`}
    >
      {children}
    </div>
  );
  ...
  return (
    <div className="preserve-3d relative mx-auto w-full max-w-[760px] animate-dashboard-float rounded-[2rem] border border-white/70 bg-white/90 p-3 shadow-[0_35px_90px_rgba(15,23,42,0.20)] backdrop-blur-xl">
      <div className="rounded-[1.55rem] border border-slate-200 bg-slate-950 p-3">
        ...
        <div className="grid gap-3 rounded-[1.25rem] bg-[#F4F6F9] p-4 md:grid-cols-[0.82fr_1.2fr_0.9fr]">
...
function PhoneMockup({ className = '' }: { className?: string }) {
  return (
    <div className={`preserve-3d relative h-[560px] w-[280px] rounded-[2.6rem] border-[10px] border-slate-950 bg-slate-950 shadow-[0_32px_70px_rgba(15,23,42,0.35)] ${className}`}>
      ...
      <div className="h-full overflow-hidden rounded-[1.85rem] bg-[#F3F5F8]">
        <div className="bg-white px-5 pb-4 pt-10">
          ...
          <p className="mt-2 text-xs leading-5 text-slate-500">Readiness is high. Last top set moved fast, so push the load and keep rest strict.</p>
        </div>
...
export default function Home() {
  ...
  const faqs = [
    ...
    {
      question: 'How do my clients get the app?',
      answer: 'You invite them from your dashboard. They download the Propel iOS app, sign in, and everything you program — training, meals, check-ins, habits — shows up there, with your branding on the experience.',
    },
    {
  ...
  return (
    <>
      <main className="min-h-screen overflow-hidden bg-[#F3F5F8] text-slate-950">
        ...
        <section className="relative px-5 pb-24 pt-32 lg:px-8 lg:pb-32 lg:pt-40">
          <div className="absolute left-1/2 top-24 h-[680px] w-[900px] -translate-x-1/2 rounded-full bg-[radial-gradient(circle,rgba(15,168,156,0.18),rgba(255,255,255,0)_62%)]" />
          <div className="absolute right-[-110px] top-48 h-72 w-72 rounded-full bg-[#0FA89C]/15 blur-3xl" />
          <div className="absolute bottom-12 left-[-90px] h-72 w-72 rounded-full bg-[#2BD4C5]/15 blur-3xl" />
          ...
          <div className="relative mx-auto grid max-w-7xl items-center gap-14 lg:grid-cols-[0.9fr_1.1fr]">
            <div>
              ...
              <h1 className="max-w-4xl text-5xl font-black tracking-[-0.055em] text-slate-950 sm:text-6xl lg:text-7xl">
                Coaching that happens every day, not just check-in day.
              </h1>
        ...
        <section id="platform" className="px-5 py-24 lg:px-8">
          <div className="mx-auto max-w-7xl">
            <AnimatedSection className="mx-auto max-w-3xl text-center">
              ...
              <p className="mt-5 text-lg leading-8 text-slate-600">Propel spans the full coaching loop: prescribe, execute, adapt, communicate, and prove progress — for you and for every client.</p>
            </AnimatedSection>
        ...
        <section id="ai" className="relative overflow-hidden bg-slate-950 px-5 py-24 text-white lg:px-8">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_25%_10%,rgba(15,168,156,.25),transparent_32%),radial-gradient(circle_at_80%_55%,rgba(43,212,197,.14),transparent_34%)]" />
            ...
            <AnimatedSection>
              ...
              <div className="mt-8 grid gap-3 sm:grid-cols-2">
                {['Start today’s workout', 'Log meal or swap it', 'Complete habit', 'Send check-in', 'Review progress', 'Message coach'].map((item) => (
                  <div key={item} className="rounded-2xl bg-[#F3F5F8] px-4 py-4 text-sm font-black text-slate-800">{item}</div>
                ))}
              </div>
            </AnimatedSection>
          ...
          <div className="mx-auto max-w-7xl">
            ...
            <div className="mt-14 grid gap-6 lg:grid-cols-3">
              {[
                ...
                <AnimatedSection key={tier.name} className={`relative rounded-[2rem] border p-7 shadow-xl ${featured ? 'border-teal-700 bg-slate-950 text-white shadow-teal-950/20' : 'border-slate-100 bg-[#F8FAFC] text-slate-950 shadow-slate-900/5'}`}>
        ...
        <section className="px-5 pb-24 lg:px-8">
          <AnimatedSection className="mx-auto max-w-6xl overflow-hidden rounded-[2.5rem] bg-slate-950 p-8 text-center text-white shadow-2xl shadow-slate-950/20 sm:p-14">
            <div className="mx-auto max-w-3xl">
              <div className="text-sm font-black uppercase tracking-[0.22em] text-teal-300">One connected platform</div>
              <h2 className="mt-4 text-4xl font-black tracking-[-0.04em] sm:text-5xl">Plan, coach, adapt, message, measure, and get paid.</h2>
              <p className="mt-5 text-lg leading-8 text-slate-300">One workspace for you, one app for your clients, and an AI layer that keeps them coached between your check-ins. Paid coach plans include a 14-day free trial.</p>
